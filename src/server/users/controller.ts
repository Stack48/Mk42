import { prisma } from "@/lib/prisma";
import { AppError } from "@/lib/errors";
import { UserInputSchema } from "@/schemas/user";

function parseInput(body: unknown) {
  const result = UserInputSchema.safeParse(body);
  if (!result.success) throw new AppError(400, result.error.issues[0].message);
  return result.data;
}

export const UsersController = {
  async findAll() {
    return prisma.utilisateur.findMany({ orderBy: { createdAt: "desc" } });
  },

  async findById(id: string) {
    const user = await prisma.utilisateur.findUnique({ where: { id } });
    if (!user) throw new AppError(404, "Introuvable");
    return user;
  },

  async create(body: unknown) {
    const { email, clerkId } = parseInput(body);
    try {
      return await prisma.utilisateur.create({ data: { email, clerkId } });
    } catch {
      throw new AppError(409, "Email déjà utilisé");
    }
  },

  async update(id: string, body: unknown) {
    const { email, clerkId } = parseInput(body);
    try {
      return await prisma.utilisateur.update({ where: { id }, data: { email, clerkId } });
    } catch {
      throw new AppError(409, "Mise à jour impossible (email déjà utilisé ?)");
    }
  },

  async remove(id: string) {
    try {
      await prisma.utilisateur.delete({ where: { id } });
    } catch {
      throw new AppError(404, "Utilisateur introuvable");
    }
  },
};
